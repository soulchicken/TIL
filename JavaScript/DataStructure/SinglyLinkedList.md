...
reverse() {
  ...
  for (let i = 0; i < this.length; i++) {
    ...
    // .next를 prev로 끼워넣어준다.
    node.next = prev;
    // prev는 다음 단계(node)로 옮겨준다.
    prev = node;
    // 해당노드도 다음 단계(next)로 옮겨준다.
    node = next;
  }
  return this;
}
...
- 링크드 리스트를 읽기 번거롭기 때문에 만든 효율성따지지 않는 함수
- 오로지 출력을 위해 존재함

```jsx
// 링크드 리스트를 한 눈에 보기 힘드니까 만든 함수
  print() {
    const arr = [];
    let current = this.head;
    while (current) {
...
- **Searching** - `O(n)`
- **Access** - `O(n)`
- 단방향 연결 리스트의 경우 삽입과 삭제에서 배열에 비해 우수하다.
- (인덱스를 사용한) 임의의 접근 작업을 하지 않는다면, 그저 주어진 **순서대로** 데이터를 관리해야한다면 적합하다.
...
- **Arrays contain a built in index whereas Linked Lists do not**
  - 인덱스 접근하는 방식에는 적합하지 않음
- **The idea of a list data structure that consists of nodes is the foundation for other data structures like Stacks and Queues**
  - 스택, 큐 자료구조를 배우는데에 빌드업이 될 것임
